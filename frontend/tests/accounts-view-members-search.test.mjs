...
assert.match(source, /命中成员：\{\{ account\.matchedMemberLabel \}\}/, '账号列表应显示命中成员提示')

assert.match(apiSource, /memberSearch\?: string/, '账号列表参数应支持成员搜索字段')
assert.match(apiSource, /matchedMemberLabel\?: string/, '账号信息类型应包含命中成员文案字段')
assert.match(apiSource, /matchedMemberCount\?: number/, '账号信息类型应包含命中成员数量字段')
